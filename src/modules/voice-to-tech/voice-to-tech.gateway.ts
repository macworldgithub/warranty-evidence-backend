  ...
  implements OnApplicationBootstrap, OnApplicationShutdown
{
  ...
  onApplicationBootstrap() {
    ...
    server.on('upgrade', (request: IncomingMessage, socket: any, head: Buffer) => {
      try {
        ...
        if (url.pathname === '/api/v1/voice-to-tech/live') {
          this.wss!.handleUpgrade(request, socket, head, (ws) => {
            this.wss!.emit('connection', ws, request);
          });
        }
      } catch (err: any) {
        this.logger.error(`Upgrade handler error: ${err.message}`);
      }
    });
  ...
  private handleClientConnection(clientWs: WebSocket) {
    ...
    try {
      ...
      dgWs.on('message', (msg: any) => {
        if (clientWs.readyState === WebSocket.OPEN) {
          try {
            ...
            if (alt && (alt.transcript || parsed.is_final)) {
              clientWs.send(
                JSON.stringify({
                  type: 'transcript',
      ...
      clientWs.on('close', () => {
        this.logger.log('Client disconnected from live stream');
        if (dgWs && dgWs.readyState === WebSocket.OPEN) {
          try {
            dgWs.send(JSON.stringify({ type: 'CloseStream' }));
            dgWs.close();
      ...
      dgWs.on('close', () => {
        if (clientWs.readyState === WebSocket.OPEN) {
          clientWs.send(JSON.stringify({ type: 'stream_closed' }));
          clientWs.close();
        }
      });
