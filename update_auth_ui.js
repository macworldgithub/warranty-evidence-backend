...
// 3. Update app/(portal)/login/page.tsx with Professional Radio-based Auth UI (Sign In & Sign Up)
writeFile('app/(portal)/login/page.tsx', `
'use client';
    ...
    try {
      if (authMode === 'signin') {
        ...
        if (response && response.user) {
          localStorage.setItem('booran_user', JSON.stringify(response.user));
          if (response.accessToken) {
            localStorage.setItem('booran_jwt', response.accessToken);
          }
          router.push(response.user.role === 'TECHNICIAN' ? '/cases/new' : '/dashboard');
        } else {
          // Fallback mock session for role
          const fallbackUser = {
          ...
          localStorage.setItem('booran_user', JSON.stringify(fallbackUser));
          router.push(selectedRole === 'TECHNICIAN' ? '/cases/new' : '/dashboard');
        }
      } else {
        // Sign Up Mode
        if (!name.trim()) {
        ...
        localStorage.setItem('booran_user', JSON.stringify(data.user));
        if (data.accessToken) {
          localStorage.setItem('booran_jwt', data.accessToken);
        }
...
console.log('✅ Updated Auth UI with Radio Buttons and 2 Roles (ADMIN & TECHNICIAN)');
